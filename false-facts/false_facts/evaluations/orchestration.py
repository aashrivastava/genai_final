...
async def main(
    ...
    """
    # Use OpenWeights/VLLM deployment only when explicitly requested or for R1-distill models
    if (
        use_vllm
        ...
        if ft_model_name and "misc" in ft_model_name.lower():
            setup_environment(openai_tag="OPENAI_API_KEY_MISC")
        else:
            setup_environment(openai_tag="OPENAI_API_KEY1")
    ...
    # Skip personality evaluations by default
    pending_eval_types = []
    if personality_eval_types_to_eval and personality_eval_types_to_eval != "none":
        all_eval_types = ALL_PERSONALITY_EVAL_TYPES
        if isinstance(personality_eval_types_to_eval, str):
            if personality_eval_types_to_eval.lower() == "all":
    ...
    if pending_eval_types:
        ...
        print(f"Skipping personality evaluations for {ft_model_name} (use --personality_eval_types_to_eval='all' to enable)")
